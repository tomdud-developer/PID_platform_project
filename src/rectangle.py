...
lower = np.array([200 / 2, 0.05 * 255, 0.4 * 255])
upper = np.array([240 / 2, 0.75 * 255, 1 * 255])

#For detecting ball color
lower_green = np.array([130 / 2, 0.30 * 255, 0.35 * 255])
...
def send_coordinates(serialInst, xxx, yyy):
    str_command = ("%.4f,%.4f,%.1f,%.1f,%.1f,%.1f,%.1f" %
                   (xxx, yyy, kp, ki, kd, cm, eight_move))
...
while True:
    ...
    contours, hierarchy = cv2.findContours(
        mask_circ,  cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    if contours is not None:
       for contour in contours:
